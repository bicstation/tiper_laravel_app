...
@section('content') {{-- app.blade.php の @yield('content') に挿入される --}}
    <div class="container mx-auto p-4"> {{-- app.blade.php がすでにコンテナを持っている場合は調整 --}}
        <h1 class="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-200">
            <i class="fas fa-tags mr-3"></i>{{ $category->name }} の商品一覧
        </h1>

        @if($products->isEmpty())
            <p class="text-gray-600 dark:text-gray-400">このカテゴリにはまだ商品が登録されていません。</p>
        @else
        ...
        <div class="mt-8 pt-4 border-t border-gray-200 dark:border-gray-700"> {{-- 戻るリンクの上に罫線を追加 --}}
            <a href="{{ route('categories.index') }}" class="text-blue-500 dark:text-blue-400 hover:underline flex items-center">
                <i class="fas fa-arrow-left w-4 h-4 mr-2"></i> カテゴリ一覧へ戻る
            </a>
        </div>
    </div>
@endsection